...
`kdc-plus install` and `kdc-plus outdated` will use the package managers
declared in this object to install *(or outdated check)* the requirements for
this KDApp. The currently supported package managers are lsited below:

 - node

This may seem a bit odd/silly, so if you seek understanding on the reasoning
behind this feature set, please see [the ABOUT page](ABOUT.md).
